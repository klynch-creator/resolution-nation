...
export default function IepPage() {
  ...
  // ── Share with Parent / Send to Inbox (Phase 11)
  const [togglingGoalId, setTogglingGoalId] = useState<string | null>(null);
  const [approvedParentId, setApprovedParentId] = useState<string | null>(null);
  const [sendingToInbox, setSendingToInbox] = useState(false);
  const [sentToInbox, setSentToInbox] = useState(false);
  const [sendInboxError, setSendInboxError] = useState<string | null>(null);
  ...
  // Load approved parent link for this student (Phase 11)
  const loadApprovedParent = useCallback(async (supabase: ReturnType<typeof createClient>) => {
    const { data } = await supabase
      ...
      .limit(1)
      .single();
    if (data) setApprovedParentId(data.parent_id as string);
  }, [studentId]);
  ...
  useEffect(() => {
    async function load() {
      ...
      await loadApprovedParent(supabase);
      setLoading(false);
    }
    load();
  ...
  // ── Phase 11: Toggle shared_with_parent ─────────────────────────────────────

  async function toggleShareWithParent(goalId: string, currentValue: boolean) {
    setTogglingGoalId(goalId);
    const supabase = createClient();
    await supabase
      .from("iep_goals")
      .update({ shared_with_parent: !currentValue })
      .eq("id", goalId);

    setIepGoals((prev) =>
      prev.map((g) =>
        g.id === goalId ? { ...g, shared_with_parent: !currentValue } : g
      )
    );
    setTogglingGoalId(null);
  }

  // ── Phase 11: Send parent update to inbox ───────────────────────────────────

  async function handleSendToParentInbox() {
    if (!parentUpdate || !approvedParentId) return;
    setSendingToInbox(true);
    setSendInboxError(null);

    const res = await fetch("/api/send-parent-message", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        parentId: approvedParentId,
        studentId,
        title: `Progress Update for ${student?.full_name ?? "your child"}`,
        bodyEnglish: parentUpdate.english,
        bodySpanish: parentUpdate.spanish,
      }),
    });

    const data = await res.json();
    if (!res.ok) {
      setSendInboxError(data.error ?? "Failed to send message.");
    } else {
      setSentToInbox(true);
    }
                        ...
                        <div className="flex flex-col gap-3">
                          {iepGoals.map((goal) => (
                            <SavedGoalCard
                              key={goal.id}
                              goal={goal}
                              toggling={togglingGoalId === goal.id}
                              onToggleShare={() =>
                                toggleShareWithParent(goal.id, goal.shared_with_parent)
                              }
                              hasParent={approvedParentId !== null}
                            />
                          ))}
                        </div>
                      )}
                  ...
                  <div className="flex gap-2">
                    ...
                    {approvedParentId ? (
                      sentToInbox ? (
                        ...
                        >
                          ✓ Sent to Parent Inbox
                        </span>
                      ) : (
                        <button
                          onClick={handleSendToParentInbox}
                          disabled={sendingToInbox}
                          style={{
                            background: sendingToInbox ? "#E2E8F0" : "#0C2340",
                            border: "none", color: "white", borderRadius: "8px",
                            padding: "0.5rem 1rem", fontSize: "0.875rem",
                            fontWeight: 600, cursor: sendingToInbox ? "default" : "pointer",
                          }}
                        >
                          {sendingToInbox ? "Sending…" : "📬 Send to Parent Inbox"}
                        </button>
                      )
                    ) : (
                      <span style={{ fontSize: "0.8125rem", color: "#94A3B8", alignSelf: "center" }}>
                        No approved parent link
                      </span>
                    )}
                  </div>
                  {sendInboxError && (
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        {goal.shared_with_parent && (
          <span
            style={{
              background: "#F0FAFA", color: "#028090", borderRadius: "100px",
              padding: "0.125rem 0.625rem", fontSize: "0.75rem", fontWeight: 600,
            }}
          >
            👪 Shared with parent
        ...
        {hasParent && (
          <button
            ...
            style={{
              ...
              borderRadius: "6px",
              border: "1.5px solid",
              borderColor: goal.shared_with_parent ? "#028090" : "#E2E8F0",
              background: goal.shared_with_parent ? "#F0FAFA" : "white",
              color: goal.shared_with_parent ? "#028090" : "#64748B",
              cursor: toggling ? "default" : "pointer",
              transition: "all 0.15s",
            }}
          >
            {toggling ? "…" : goal.shared_with_parent ? "✓ Shared" : "Share with parent"}
          </button>
        )}
      </div>
    </div>
